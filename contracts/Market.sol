...
contract NFTMarket is ReentrancyGuard {
  ...
  function transferTo(address nftContract, address to, uint256 tokenId) public {
    IERC721(nftContract).transferFrom(msg.sender, address(to), tokenId);
    uint totalItemCount = _itemIds.current();
    for (uint i = 0; i < totalItemCount; i++) {
      if (idToMarketItem[i + 1].tokenId == tokenId) {
        idToMarketItem[i + 1].owner = payable(to);
      }
    }
  }
